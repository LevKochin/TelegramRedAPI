...
public class TelegramService(IHttpClientFactory clientFactory, IHostEnvironment env)
{
    ...
    public async Task<IResult> UpdateWebhook(string? url)
    {
        try
        {
            if (string.IsNullOrEmpty(url))
            {
                return Results.Json(new ResponseMessage(-1, "Отсутствует url"));
            }
        ...
        catch (Exception ex)
        {
            return env.IsProduction()
                ? Results.Json(new ResponseMessage(-1,
                    "Возникла системная ошибка, пожалуйста, обратитесь к администратору"))
                : Results.Json(new ResponseMessage(-1, ex.Message));
        }
    }
    ...
    public async Task<int> StartPageGenerating(long chatId, int messageId) =>
        await EditButtons(ConvertToJson(new
        {
            chat_id = chatId,
            text = "Вы инициировали процесс генерации, начинаем с инициации методанных. Вы можете вводить их постепенно",
            ...
            reply_markup = new
            {
                inline_keyboard = new[]
                {
                    new[]
                    {
                        ...
                        new { text = " Метаданные", callback_data = "backToMain" },

                    },
                    new[]
                    {
                        new { text = " Контент", callback_data = "backToMain" },
                        new { text = " Источники", callback_data = "backToMain" }
                    }
                }
            }
        }));
